...
class Stage1Config:
    ...
    # Cached features. If set, skip foundation forward passes entirely —
    # read precomputed (gte, clip_text, clip_vision, vjepa_patches) tensors
    # from disk and gather by sample.idx in the training loop. Each step
    # becomes a tensor slice + adapter MLPs (~ms) instead of a JPG decode +
    # V-JEPA-2 forward (~seconds). Generated by scripts/precompute_features.py.
    cache_dir: str = ""
...
class Stage1Trainer:
    ...
    def __init__(
        ...
        # --- Cache mode? ---
        # If cfg.cache_dir is set, we skip loading foundation models entirely.
        # All features come from precomputed tensors on disk.
        self.use_cache = bool(cfg.cache_dir)
        self._cache: dict[str, torch.Tensor] = {}

        if self.use_cache:
            self._load_cache(Path(cfg.cache_dir))
            # Use stored native dims for adapter shapes; foundations not loaded.
            from selflearnai.foundations import (
                CLIP_NATIVE_DIM, GTE_NATIVE_DIM, VJEPA2_NATIVE_DIM,
            )
            self._gte_dim = GTE_NATIVE_DIM
            self._clip_dim = CLIP_NATIVE_DIM
            self._vjepa_dim = VJEPA2_NATIVE_DIM
        else:
            # --- Frozen foundations (live encoding path) ---
            ...
            self._gte_dim = self.gte.native_dim
            self._clip_dim = self.clip.native_dim
            self._vjepa_dim = self.vjepa.native_dim

        # --- Trainable adapters ---
        self.bundle = AdapterBundle(
            clip_dim=self._clip_dim,
            gte_dim=self._gte_dim,
            vjepa_dim=self._vjepa_dim,
    ...
    # -----------------------------------------------------------------------
    # Load precomputed feature cache (called when cfg.cache_dir is set)
    # -----------------------------------------------------------------------
    def _load_cache(self, cache_dir: Path) -> None:
        text_path = cache_dir / "text_features.pt"
        img_path = cache_dir / "image_features.pt"
        if not text_path.exists() or not img_path.exists():
            raise FileNotFoundError(
                f"Cache files not found in {cache_dir}. "
                f"Run scripts/precompute_features.py first."
            )
        print(f"Loading precomputed features from {cache_dir} ...")
        text_blob = torch.load(text_path, map_location="cpu", weights_only=False)
        img_blob = torch.load(img_path, map_location="cpu", weights_only=False)

        # Move to training device. Cast to float32 for training-time math
        # (precompute may have been fp16 to halve disk).
        dev = self.cfg.device
        self._cache["gte"] = text_blob["gte"].to(dev).float()
        self._cache["clip_text"] = text_blob["clip_text"].to(dev).float()
        self._cache["sample_to_image_idx"] = text_blob["sample_to_image_idx"].to(dev)
        self._cache["clip_vision"] = img_blob["clip_vision"].to(dev).float()
        self._cache["vjepa_patches"] = img_blob["vjepa_patches"].to(dev).float()
        n_t = self._cache["gte"].shape[0]
        n_i = self._cache["clip_vision"].shape[0]
        gb = sum(t.element_size() * t.numel() for t in self._cache.values()) / 1e9
        print(f"  cached: {n_t} text features × {n_i} image features  ({gb:.2f} GB on {dev})")
    ...
    def step(self, step_idx: int) -> dict:
        ...
        if self.use_cache:
            # Cache path: gather precomputed features by sample.idx and
            # sample_to_image_idx mapping. Zero foundation work per step.
            sample_indices = torch.tensor(
                [s.idx for s in batch], dtype=torch.long, device=cfg.device,
            )
            img_indices = self._cache["sample_to_image_idx"].index_select(
                0, sample_indices,
            )
            z_gte = self._cache["gte"].index_select(0, sample_indices)
            z_clip_text = self._cache["clip_text"].index_select(0, sample_indices)
            z_clip_vision = self._cache["clip_vision"].index_select(0, img_indices)
            z_vjepa_patches = self._cache["vjepa_patches"].index_select(0, img_indices)
        else:
            # Live encoding path: read images from disk, run foundations.
    ...
    def anchor_drift_cosine(self) -> float:
        ...
        Only meaningful when freeze_anchor=False; we still expose it as a
        sanity check. With cache mode, we use the precomputed CLIP-text
        features directly as probes.
        """
        ...
        a_init = Adapter(self._clip_dim, SHARED_DIM).to(self.cfg.device)
        a_init.load_state_dict(probe)
        a_init.eval()

        # Probe inputs: 32 CLIP-text features.
        if self.use_cache:
            n = self._cache["clip_text"].shape[0]
            probe_indices = torch.randperm(n, device=self.cfg.device)[:32]
            z_clip_text = self._cache["clip_text"].index_select(0, probe_indices)
            # Note: cache stored *post-CLIP* (512-dim), but anchor_c expects
            # CLIP-output (also 512-dim). Same shape — works directly.
        else:
            import random as _r
            rng = _r.Random(0)
